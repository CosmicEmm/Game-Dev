...
car_rect = car.get_rect()                    # creates a Rect object that represents the rectangular area of the car image
car_rect.center = right_lane, height * 0.8   # y-coordinate: bottom of the screen

# Load Enemy Vehicle
car2 = pygame.image.load('otherCar.png')
car2_rect = car.get_rect()
car2_rect.center = left_lane, height * 0.2   # y-coordinate: 'height * 0.2' means top of the screen
...
while running:
    ...
    car2_rect[1] += speed                    # move the enemy vehicle downward and out of sight
    if car2_rect[1] > height:
        # Randomnly Select Lane
        if random.randint(0, 1) == 0:
            car2_rect.center = right_lane, -200 # place the enemy vehicle's center 200px above the top edge of the right lane, creating a looping effect
        else:
            car2_rect.center = left_lane, -200
    
    # End Game Logic
    if car_rect.colliderect(car2_rect):   # The colliderect() method checks if two Rect objects overlap
    ...
    for event in pygame.event.get():      # The get method fetches all the events (e.g., keyboard input, mouse clicks, window close)
        ...
        if event.type == KEYDOWN:         # selects all the keys of our keybaord
            if event.key == K_ESCAPE:     # The user presses the esc key to execute pygame.quit() - also works in fullscreen mode
                running = False
            # Lane Logic and Edge Cases
            if event.key in [K_a, K_LEFT] and car_rect.center == (right_lane, height * 0.8): 
                car_rect = car_rect.move([- int(road_w/2), 0])   # Move User Car to the Left
            else:                                                # Edge Case: if A/LEFT is pressed and car is already in left lane, do nothing                                       
                None                                                
            if event.key in [K_d, K_RIGHT] and car_rect.center == (left_lane, height * 0.8): 
                car_rect = car_rect.move([+ int(road_w/2), 0])   # Move User Car to the Right
